...
- [Vista Previa de los Mailables en el Navegador](#previewing-mailables-in-the-browser)
...
Laravel proporciona una API limpia y simple sobre la popular libreria [SwiftMailer](https://swiftmailer.symfony.com/) con drivers para SMTP, Mailgun, SparkPost, Amazon SES, las funciones de PHP `mail` y `sendmail`, 
permitiendote comenzar rápidamente a enviar correos a través de un servicio local o en la nube de tu elección.
...
El componente de botón representa un enlace de botón centrado. El componente acepta dos argumentos, una `url` y un `color` opcional. Los colores compatibles son `blue`, `verde` y `red`. Puedes agregar los botones que desees a un mensaje:
...
## Visualizar Mailables en el Navegador

Al diseñar una plantilla de mailables, es conveniente obtener una vista previa rápida en su navegador como una típica plantilla Blade. Por esta razón, Laravel le permite devolver cualquier mailable directamente desde un retorno de alguna ruta o controlador. Cuando se devuelve un mailable, se procesará y se mostrará en el navegador, lo que le permite obtener una vista previa rápida de su diseño sin necesidad de enviarlo a una dirección de correo electrónico real:
...
Por supuesto, no está limitado a especificar los destinatarios "a" al enviar un mensaje. Usted es libre de configurar los destinatarios "a", "cc" y "bcc", todo dentro de una única llamada a un método encadenado:
...
Este método se encargará automáticamente de insertar un queue en la cola para que el mensaje se envíe en segundo plano. Por supuesto, necesitarás [configurar tus queues](/docs/{{version}}/queues) antes de usar esta característica.
...
Si tiene clases mailable que desees que estén siempre en cola, puedes implementar el contrato `ShouldQueue` en la clase. Ahora, incluso si llamas al método `send` cuando envies correos el mailable seguirá en cola ya que implementa el contrato:
