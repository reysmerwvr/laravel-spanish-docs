...
- [Controlador `RegisterController`](#the-register-controller)
...
Si estás sobreescribiendo los métodos `register` o `registered` incluidos en el `RegisterController` de Laravel, debe asegurarte de llamar `parent::register` y `parent::registered` desde sus respectivos métodos. El envío del evento `Illuminate\Auth\Events\Registered` y el inicio de sesión del nuevo usuario registrado se han trasladado al método `registered`, por tanto si estás sobreescribiendo este método y no está llamando al metodo padre, el proceso de registro de usuario fallará.
